...
import tiger.common.dal.dataobject.AccountBindDO;
import tiger.common.dal.dataobject.example.AccountBindExample;
import tiger.common.dal.enums.AccountSocialTypeEnum;
import tiger.common.dal.persistence.AccountBindMapper;
import tiger.core.domain.AccountBindDomain;
import tiger.core.domain.convert.AccountBindConvert;
import tiger.core.enums.ErrorCodeEnum;
import tiger.core.exception.TigerException;
import tiger.core.service.AccountBindService;
...
public class AccountBindServiceImpl implements AccountBindService {
    ...
    /**
     * @see AccountBindService#findAccountByUnionId(String, AccountSocialTypeEnum)
     */
    @Override
    public AccountBindDomain findAccountByUnionId(String unionId, AccountSocialTypeEnum type) {
        AccountBindExample example = new AccountBindExample();

        example.createCriteria().andUnionIdEqualTo(unionId).andAccountTypeEqualTo(type.getCode());

        List<AccountBindDO> bindDOs = accountBindMapper.selectByExample(example);
        if (bindDOs.size() == 0) {
            return null;
        } else if (bindDOs.size() > 1) {
            logger.error("[AccountBind-Duplicate] 检查到账号重复绑定 unionId:[" + unionId + "], " +
                    "Type : [" + type.getCode() + "]");
            throw new TigerException(ErrorCodeEnum.BIZ_DUPLICATIVE, "检查到该账号重复绑定");
        } else {
            return AccountBindConvert.convert2Domain(bindDOs.get(0));
